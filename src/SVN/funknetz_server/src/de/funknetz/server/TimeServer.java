...
public class TimeServer {
	...
	**/
	public void setTimeTable (String[][] timeTable_ref) {
		...
		Calendar c_ref;
		String uid_ref;
		for (int i = 0; i < timeTable_ref.length && timeTable_ref[i] != null; i++) {
			...
			uid_ref = new java.rmi.server.UID().toString();
			
			Object[] line_ref = {c_ref, timeTable_ref[i][2], timeTable_ref[i][3], timeTable_ref[i][4], timeTable_ref[i][5], timeTable_ref[i][6], uid_ref};
				
			timeTable_class[i] = line_ref.clone();
		} //endfor
		write();
		scheduleTimer();
	...
	**/
	private void scheduleTimer() {
		...
		Calendar c_ref;
		long period = 0L;
		for (int i = 0; i < timeTable_class.length && timeTable_class[i] != null; i++) {
			TimeEvent myEvent_ref = new TimeEvent((String)timeTable_class[i][6]);	
			c_ref = (GregorianCalendar) timeTable_class[i][0];
	...
	**/
	class TimeEvent extends TimerTask {
		...
		public void run() {
			//while (TimeServer.isLocked_class == true) {
			//	try {
			...
			//	} //endtry
			//} //endwhile
			//TimeServer.isLocked_class = true;
			for (int i = 0; i < timeTable_class.length; i++) {
				if (timeTable_class[i] != null && ((String)timeTable_class[i][6]).equals(uid_ref)) {
					execute(timeTable_class[i]);
					timeTable_class[i] = null;
				} //endif
			} //endfor
			//TimeServer.isLocked_class = false;
			this.cancel();
			write();
			executionTimer_ref.purge();
		} //endmethod run

...
public class TimeEvents {
	...
	private JTextField timeF_ref;					// Serveruhr, wird zu beginn synchronisiert
	private static Calendar serverTime_class;		// Die aktuelle Serverzeit
	private java.util.Timer serverTTimer_ref;			// Der Timer fuer die Serverzeit
	...
	**/
	public void draw() {
		...
		JButton einlesenB_ref = new JButton("Einlesen");
		einlesenB_ref.addActionListener(new ButtonBox1Listener());
		JButton bearbeitenB_ref = new JButton("Bearbeiten");
		bearbeitenB_ref.addActionListener(new ButtonBox1Listener());
		JButton loeschenB_ref = new JButton("Loeschen");
		...
		JButton cpZURUECKSETZENbutton_ref = new JButton("Zuruecksetzen");
		cpZURUECKSETZENbutton_ref.addActionListener(new CreatorButtonListener());
		JButton cpNEUbutton_ref = new JButton("Neu");
		cpNEUbutton_ref.addActionListener(new CreatorButtonListener());
		JButton cpSAVEbutton_ref = new JButton("Speichern");
		...
		JButton closeB_ref = new JButton("Zurueck");
		closeB_ref.addActionListener(new ButtonBox2Listener());
		JButton takeoB_ref = new JButton("Uebernehmen");
	...
	**/
	private void initializeServerTimer() {
		serverTTimer_ref = new java.util.Timer();
		serverTTimer_ref.scheduleAtFixedRate(new MyServerTimer(), new Date(), 1000L);
	...
	**/
	class MyServerTimer extends TimerTask {
		public void run() {
			...
			timeF_ref.setText(String.format("%tT", serverTime_class));
		} //endmethod run
	} //endclass EventTimer
	...
	class ServerConnector implements Runnable {
		public void run() {
			try {
				...
				oos_ref.close();
				s_ref.close();
				if (commandType_ref.equals("get") && felderMatrix_ref[0][0] != null) {
					tableMod_ref.setDataVector(felderMatrix_ref, TABLENAMES);
					tableMod_ref.fireTableDataChanged();
				} else if (commandType_ref.equals("get") && felderMatrix_ref[0][0] == null) {
	...
	**/
	class CreatorButtonListener implements ActionListener {
		public void actionPerformed (ActionEvent e_ref) {
			if (e_ref.getActionCommand().equals("Zuruecksetzen")) {

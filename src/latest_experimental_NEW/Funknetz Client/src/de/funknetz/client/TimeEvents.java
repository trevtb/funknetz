...
public class TimeEvents {
	...
	private String[][] felderMatrix_ref;						// der beschnittene Inhalt der Tabelle in String-Form.
	...
	private final String[] TABLENAMES = {"Datum", "Uhrzeit", "Name", "Kanal", "Schalter", "Befehl", "Intervall", "Typ"};
	private TimeEventHelper timeHelper_ref;
	...
	**/
	public TimeEvents() {
		...
		commandType_ref = "get";
		tableMod_ref = (DefaultTableModel) new MyTableModel(null, TABLENAMES);
		connect();
	...
	**/
	private void connect() {
		if (commandType_ref.equals("get")) {
			...
			System.out.println("Status: "+status_ref);
			...
			} else if (status_ref.equals("success") && felderMatrix_ref != null) {
				serverTime_class = (Calendar)tempA_ref[1];
				felderMatrix_ref = (String[][])tempA_ref[2];
			...
			status_ref = timeHelper_ref.setTimeEvents(felderMatrix_ref);
			if (status_ref.equals("success")) {
			...
			} else if (status_ref.equals("connection_error")) {	
				JOptionPane.showMessageDialog(frame_ref, "Es trat ein Problem mit der Verbindung zum Server auf, versuchen Sie es erneut.", "Verbindungsfehler", JOptionPane.ERROR_MESSAGE);
			} //endif
		} //endif
	} //endmethod connect
	...
	class CreatorButtonListener implements ActionListener {
		public void actionPerformed (ActionEvent e_ref) {
			if (e_ref.getActionCommand().equals("Jetzt!")) {
				...
				String[] newEvent_ref = new String[8];

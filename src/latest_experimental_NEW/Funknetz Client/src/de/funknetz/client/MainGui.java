...
public class MainGui {
	...
	public void draw() {
		...
		JPanel coffeePan_ref = new JPanel();
		coffeePan_ref.setLayout(new BoxLayout(coffeePan_ref, BoxLayout.Y_AXIS));
		ImageIcon coffeeIcon_ref = new ImageIcon(ClassLoader.getSystemResource("de/funknetz/client/resource/coffee_m.gif"));
		JButton coffeeBut_ref = new JButton(coffeeIcon_ref);
		coffeeBut_ref.setActionCommand("Coffee");
		coffeeBut_ref.addActionListener(new MainListener());
		coffeeBut_ref.setAlignmentX(Component.CENTER_ALIGNMENT);
		Dimension d5_ref = new Dimension(coffeeIcon_ref.getIconWidth(), coffeeIcon_ref.getIconHeight());
		coffeeBut_ref.setPreferredSize(d5_ref);
		JLabel coffeeLab_ref = new JLabel("Kaffee");
		coffeeLab_ref.setAlignmentX(Component.CENTER_ALIGNMENT);
		coffeePan_ref.add(coffeeBut_ref);
		coffeePan_ref.add(coffeeLab_ref);
		shortcutPan_ref.add(coffeePan_ref);
	...
	class MainListener implements ActionListener {
		public void actionPerformed(ActionEvent ev_ref) {
			...
			} else if (ev_ref.getActionCommand().equals("Coffee")) {
				kaffee_ref = new Coffee();
				kaffee_ref.draw();
			} //endif

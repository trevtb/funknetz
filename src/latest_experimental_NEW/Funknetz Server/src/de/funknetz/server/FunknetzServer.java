...
public class FunknetzServer {
	...
	public static BufferedReader br_class;
	...
	**/
	public void connect() {
		try {
			...
			} else if (type_ref.equals("devicelist")) {
				System.out.println("DEVICELIST");
				String what_ref = (String)ois_class.readObject();
				if (what_ref.equals("get")) {
					System.out.println("GET");
					Device[][] deviceArray_ref = null;
					while (deviceArray_ref == null) {
						deviceArray_ref = helfer_class.getDeviceList();
					} //endwhile
					for (int i=0;i<deviceArray_ref.length;i++) {
						System.out.println("");
						for (int j=0;j<deviceArray_ref[i].length;j++) {
							System.out.print(deviceArray_ref[i][j]+" ");
						} //endfor
					} //endfor
					ous_class.writeObject(deviceArray_ref);
				} else if (what_ref.equals("set")) {
					Device[][] deviceArray_ref = (Device[][]) ois_class.readObject();
					helfer_class.setDeviceList(deviceArray_ref);
				} //endif
			} //endif
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public void connectMobile(int callValue) {
		System.out.println("Mobile connected.");
		int[] tempAr_ref = makeIt(callValue);
		execute(tempAr_ref);
	} //endmethod connectMobile
	...
	*	Hierzu wird ecine Instanz der Klasse IniHelfer genutzt.
			...
			if (useGui_class && !(gui_class.getPortTextF().equals(""))) {
				...
				if (isActive) {
					try {
						...
						success = true;
						FunknetzServerGui.setStatusText("\nserver started ...\n"); 
					} catch (Exception ex) {
		...
		public void run() {
			while (true) {
				try {
					...
					br_class = new BufferedReader(new InputStreamReader(s_ref.getInputStream()));
					String type_ref = br_class.readLine();
					char[] tempType_ref = type_ref.toCharArray();
					if (tempType_ref[4] == 'c') {
						...
						connect();
					} else if (tempType_ref[4] == 'm') {
						String tempCV_ref = tempType_ref[5] + "";
						try {
							if ((Integer.parseInt(tempType_ref[6] + "") != -1)) {
									tempCV_ref += tempType_ref[6] + "";
							} //endif
						} catch (NumberFormatException ex_ref) {
						} //endtry
						int callVal = Integer.parseInt(tempCV_ref);
						connectMobile(callVal);
					} else {
						if (useGui_class == true) {
							FunknetzServerGui.setStatusText("Recieved invalid command from client.");
							FunknetzServerGui.autoScroll();
						} else {
							System.out.println("Error: Recieved invalid command from client: " + type_ref);
						} //endif
					} //endif
